using Microsoft.Xrm.Sdk;
using System;
using Xunit;

namespace SparkCode.Tests.Text
{
    public class ParseURLTests
    {
        [Fact]
        public void ParseURL_ValidUrl_Returns_Parsed_Components()
        {
            var expectedId = Guid.NewGuid();

            string url = $"http://www.example.com/path?id={expectedId}&etn=account#fragment";
            var results = SparkCode.Text.ParseURL.Parse(url);
            var query = (Entity)results["query"];
            
            Assert.Equal("http", (string)results["scheme"]);
            Assert.Equal("www.example.com", (string)results["host"]);
            Assert.Equal(80, (int)results["port"]);
            Assert.Equal("/path", (string)results["absolutePath"]);
            Assert.Equal("fragment", (string)results["fragment"]);
            Assert.Equal(expectedId, Guid.Parse((string)query["id"]));
            Assert.Equal("account", (string)query["etn"]);
            Assert.Equal(expectedId, Guid.Parse((string)results["id"]));
            Assert.Equal("account", (string)results["etn"]);
        }

        [Fact]
        public void ParseURL_WithoutIdAndEtn_DoesNotReturn_ThoseFields()
        {
            string url = "http://www.example.com/path?query=value";
            var results = SparkCode.Text.ParseURL.Parse(url);

            Assert.False(results.Attributes.Contains("id"));
            Assert.False(results.Attributes.Contains("etn"));
        }

        [Fact]
        public void ParseURL_InvalidURL_Throws_Exception()
        {
            string url = "abc123";
            Assert.Throws<UriFormatException>(() =>
                SparkCode.Text.ParseURL.Parse(url)
            );
        }
    }
}
